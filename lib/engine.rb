...
class BattleshipEngine
  ...
  def print_boards_with_ship_locs
    print_board_with_ship_locs(@map_1, 1)
    print_board_with_ship_locs(@map_2, 2)
  end

  def print_board_with_ship_locs(map, bot_num)
    temp = map.dup
    temp.board = temp.dup_board
    temp.mark_with_ships
    puts "Bot #{bot_num}'s map"
    puts temp
  end
  ...
  def place_ships(bot, map)
    ...
    # map = generator.to_map
  end

  def one_turn(bot, map)
    shot = Shot.new bot.take_turn(map.dup_board)
